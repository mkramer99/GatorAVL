...
void parseCommand(Tree &t, istringstream &in) {
    ...
    if (command ==  "insert") {
        ...
        if (name.size() > 0 && id.size() == 8) {
            ...
            // balance tree
            int balanceFactor = t.getBalanceFactor(t.root);
            // left left- right rotation
            if (balanceFactor > 1 && t.getBalanceFactor(t.root->left) > 0) {
                t.root = t.leftLeft(t.root);
            }
            // right right - left rotation
            if (balanceFactor < -1 && t.getBalanceFactor(t.root->right) < 0) {
                t.root = t.rightRight(t.root);
            }
            // left right bf > 1 left child bf < 0
            if (balanceFactor > 1 && t.getBalanceFactor(t.root->left) < 0) {
                t.root = t.leftRight(t.root);
            }
            // right left bf < -1 right child bf > 0
            if (balanceFactor < -1 && t.getBalanceFactor(t.root->right) > 0) {
                t.root = t.rightLeft(t.root);
            }
        } else {
            cout << "unsuccessful" << endl;
        }
